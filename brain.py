...
if uploaded_file:
    # Leer la imagen y convertirla en array (en escala de grises)
    ...
    if image is not None:
        ...
        if tumor_detected:
            ...
            pixel_spacing = 0.04  # cm/píxel (ajusta según la resolución de la imagen)

            # Preprocesamiento: suavizado y umbral fijo
            ...
            if tumor_contour is not None and cv2.contourArea(tumor_contour) > min_area_threshold:
                ...
                # Convertir imagen a BGR para dibujar en color
                ...
                # Crear máscara y extraer la región del tumor
                ...
                # Mostrar segmentación y resultados
                st.image([image, cv2.cvtColor(heatmap, cv2.COLOR_BGR2RGB)], width=400, caption=["Imagen Original", "Segmentación del Tumor"])
