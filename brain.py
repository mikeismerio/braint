...
# ✅ Permitir al usuario subir imágenes en la barra lateral
uploaded_file_craneo = st.sidebar.file_uploader("📸 Imagen para Análisis del Cráneo", type=["png", "jpg", "jpeg"])
uploaded_file_tumor = st.sidebar.file_uploader("📸 Imagen para Análisis del Tumor", type=["png", "jpg", "jpeg"])

# =================== PÁGINA 1: ANÁLISIS CRANEAL ===================
if page == "Análisis Craneal" and uploaded_file_craneo:
    st.title("📏 Análisis del Cráneo")

    image_bytes = uploaded_file_craneo.read()
...
elif page == "Análisis del Tumor" and uploaded_file_tumor:
    st.title("🧠 Análisis del Tumor")

    image_bytes = uploaded_file_tumor.read()
    image_array = np.frombuffer(image_bytes, np.uint8)
    tumor_image = cv2.imdecode(image_array, cv2.IMREAD_GRAYSCALE)
